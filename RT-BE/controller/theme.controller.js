...
exports.updateThemeSetting = async function (req, res, next) {
    try {
        ...
        if(req.params.type === 'systemLogo') {
            if (req.files.favicon && req.files.favicon[0].filename != undefined) obj = { ...obj, "systemLogo.favicon": req.files.favicon[0].filename }
            if (req.files.logo && req.files.logo[0].filename !== undefined) obj = { ...obj, "systemLogo.systemLogo": req.files.logo[0].filename }
        }

        if(req.params.type === "signInPageLogo") {
            if (req.files.logo && req.files.logo[0].filename != undefined) obj = { ...obj, "signInPageLogo.logo": req.files.logo[0].filename }
        }
        if(req.params.type === "recruitmentPageLogo") {
            if (req.files.logo && req.files.logo[0].filename != undefined) obj = { ...obj, "recruitmentPageLogo.logo": req.files.logo[0].filename }
        }
        if(req.params.type === "payRollLogo") {
            if (req.files.payRollLogo && req.files.payRollLogo[0].filename != undefined) obj = { ...obj, "payRollLogo.payRollLogo": req.files.payRollLogo[0].filename }
        }
        if(req.params.type === 'organizationChart') {
            ...
            if (req.body.title !== undefined) obj = { ...obj, "organizationChart.title": req.body.title }
        }

        await db.themes.findOneAndUpdate({}, {
            $set: obj
        })
