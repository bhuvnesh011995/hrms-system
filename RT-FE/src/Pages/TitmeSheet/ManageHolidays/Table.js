...
export default function Table() {
  ...
  const updateHolidayEvents = (eventData) => {
    const filteredEvents = holidayEvents.filter(
  ...
  const deleteSelectedEvent = async () => {
    try {
      const deleteHoliday = await api.delete(
        "/events/deleteEvent/" + eventData._id,
      );
        ...
        <div className='card'>
          <div className='card-body'>
            ...
            <CommonDataTable
              ...
              actionButtons
              editButton={
                permissions.includes("All") || permissions.includes("update37")
              }
              deleteButton={
                permissions.includes("All") || permissions.includes("delete37")
              }
              callback={(data, type, index) =>
                showHolidayModal(data, type, index)
              }
            />
          </div>
        </div>
