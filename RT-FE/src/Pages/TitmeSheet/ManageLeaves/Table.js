...
export default function Table() {
  ...
  const deleteSelectedEvent = async () => {
    try {
      const deleteLeave = await api.delete(
        "/events/deleteEvent/" + leaveData._id,
      );
      if (deleteLeave.status == 200) {
        const filteredEvents = leaveEvents.filter(
          (event) => event._id != leaveData._id,
        );
    ...
    <div class='row'>
      ...
      {leaveModal && (
        <NewLeaveModal
          ...
          eventData={leaveData}
          callback={(data) => updateLeaveEvents(data)}
        />
      )}
      ...
      {leaveDelete && (
        <CommonDeleteModal
          header={"Leave"}
          show={leaveDelete}
          setShow={setLeaveDelete}
          message={`do you really want to delete leave ${leaveData?.title}`}
