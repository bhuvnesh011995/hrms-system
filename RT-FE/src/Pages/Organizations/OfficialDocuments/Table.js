...
export default function Table() {
  ...
  const columns = useMemo(
    () => [
      {
        accessorFn: (row) =>
          row.documentType ? row.documentType.name : "not available",
        id: "documentType",
      ...
      },
      {
        accessorFn: (row) =>
          row.expiryDate
            ? row.expiryDate.slice(0, 10).split("-").reverse().join("/")
            : "not available",
        id: "expiryDate",
        header: "Expiry Date",
        Header: () => (
                    ...
                    <IconButton
                      ...
                      onClick={() => {
                        let obj = {
                          ...
                          documentType: row.original.documentType._id,
                          expiryDate: row.original.expiryDate?.slice(0, 10),
                        };
                        setViewData(obj);
                        setIsOpen(true);
                      }}
