...
import { getAllTerminations,deleteTermination } from "../../../Utility/API/termination"
...
export default function EmployeeTable() {
      ...
      const columns = useMemo(() => [
        ...
        {                                                   
          ...
          {                                                   
            ...
            {                                                   
                accessorFn:  (row)=>row.noticeDate ? row.noticeDate.slice(0,10).split("-").reverse().join("/"):"not available",
             id:"noticeDate",
                 header: 'Notice Date',
               },
             
               {                                                   
                accessorFn: (row)=>row.terminationDate ? row.terminationDate.slice(0,10).split("-").reverse().join("/"):"not available",
                id:"terminationDate",
    ...
    return(
        <div className="row">
                                ...
                                <div className="card-body">
                                    <div className="row">
                                        ...
                                        <div className="col-md-6 mb-3" style={{textAlign: "right"}}>
                                            <button className="btn btn-primary text-right" data-bs-toggle="modal"onClick={()=>setIsOpen(true)} >Add New</button>
                                        </div>
                                    </div>
              ...
              renderRowActions={({ row, table }) => (
                <Box sx={{ display: "flex", flexWrap: "nowrap", gap: "8px" }}>
                  ...
                  <IconButton
                    color="secondary"
                    onClick={() => {
                        let obj = {...row.original, company:row.original?.company?._id, employee:row.original.employee?._id,noticeDate:row.original.noticeDate?.slice(0,10),terminationDate:row.original.terminationDate?.slice(0,10)}
                        setViewData(obj)
                        setIsOpen(true)
                      }}
                  >
            ...
            />
               {isOpen && <AddNewTermination   viewData={viewData} setViewData={setViewData} getTerminations={getTerminations} show={isOpen} setShow={setIsOpen} /> }
               {isViewOpen && <View show={isViewOpen} setShow={setIsViewOpen} setViewData={setViewData} viewData={viewData} />   }
