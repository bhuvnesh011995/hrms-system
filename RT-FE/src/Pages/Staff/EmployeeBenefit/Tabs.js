import { Card, Col, Nav, Row, Tab } from "react-bootstrap";
import Accommodation from "./TabsContent.js/Accommodation/Accommodation";
import AccommodateEmployee from "./TabsContent.js/AccommodateEmployee/AccommodateEmployee";

export default function Tabs() {
    return(
        <Card>
          <Card.Body>
        <Tab.Container id="left-tabs-example" defaultActiveKey="contract">
      <Row>
        <Col xl={3}>
            <Nav variant="pills" className="flex-column">
              <Nav.Item>
                <Nav.Link eventKey="accommo">Accommodation</Nav.Link>
              </Nav.Item>
              <Nav.Item>
                <Nav.Link eventKey="accEmp">Accommodate Employee</Nav.Link>
              </Nav.Item>
              <Nav.Item>
                <Nav.Link eventKey="contract">Utilities & Accessories</Nav.Link>
              </Nav.Item>
              <Nav.Item>
                <Nav.Link eventKey="contract">Driver</Nav.Link>
              </Nav.Item>
              <Nav.Item>
                <Nav.Link eventKey="contract">Housekeeping</Nav.Link>
              </Nav.Item>
              <Nav.Item>
                <Nav.Link eventKey="contract">Hotel Accommodation</Nav.Link>
              </Nav.Item>
              <Nav.Item>
                <Nav.Link eventKey="contract">Other Benefits</Nav.Link>
              </Nav.Item>
            </Nav>
        
        </Col>
        <Col xl={9}>
        
            <Tab.Content>
                <Tab.Pane eventKey={"accommo"}><Accommodation/></Tab.Pane>
                <Tab.Pane eventKey={"accEmp"}><AccommodateEmployee/></Tab.Pane>
                <Tab.Pane eventKey={"accommo"}><Accommodation/></Tab.Pane>
                <Tab.Pane eventKey={"accommo"}><Accommodation/></Tab.Pane>
                <Tab.Pane eventKey={"accommo"}><Accommodation/></Tab.Pane>
                <Tab.Pane eventKey={"accommo"}><Accommodation/></Tab.Pane>
                <Tab.Pane eventKey={"accommo"}><Accommodation/></Tab.Pane>
            </Tab.Content>
            </Col>
        </Row>
        </Tab.Container>
        </Card.Body>
        </Card>
    )
};
