...
export default function ListAllEmployees() {
  ...
  let columns = useMemo(
    () => [
      ...
      },
      {
        ...
        Cell: ({ cell }) => (
          ...
          >
            ...
            <div style={{ fontSize: ".7rem" }}>
              Location :{cell.getValue()?.location}
            </div>
            <div style={{ fontSize: ".7rem" }}>
              Department :{cell.getValue()?.department}
            </div>
            <div style={{ fontSize: ".7rem" }}>
              Designation :{cell.getValue()?.designation}
            </div>
          </Box>
        ),
      },
      ...
      {
        accessorFn: (row) =>
          row.reportTo ? `${row.reportTo.fName} ${row.reportTo.lName}` : "NA",
        Header: () => (
          <FormattedMessage id='Report_To' defaultMessage={"Report To"} />
        ),
        header: "Report To",
        size: "100",
      },
      {
        ...
        Header: () => <FormattedMessage id='Role' defaultMessage={"Role"} />,
        header: "Role",
        size: "100",
        Cell: ({ cell }) => (
          <Box
        ...
        <MaterialReactTable
          ...
          renderRowActions={({ row }) => (
            <Box sx={{ display: "flex", flexWrap: "nowrap", gap: "8px" }}>
                ...
                <IconButton
                  ...
                  onClick={() => {
                    let obj = {
                      ...
                      confirmationDate: row.original.confirmationDate?.slice(
                        0,
                        10,
                      ),
                      dateOfJoining: row.original.dateOfJoining?.slice(0, 10),
                      prEffectiveDate: row.original.prEffectiveDate?.slice(
                        0,
                        10,
                      ),
              ...
              {havePermission("delete", row.original.role?.name) ? (
                <IconButton
                  color='error'
                  onClick={async () => {
