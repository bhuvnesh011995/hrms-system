...
export default function Incom({ data, getAll, setIsError }) {
    ...
    () => [
      {
        accessorKey: "name",
        header: "Incom",
        Header: () => <FormattedMessage id='Incom' defaultMessage={"Incom"} />,
      },
    ],
    [],
  );
            ...
            renderRowActions={({ row, table }) => (
              <Box sx={{ display: "flex", flexWrap: "nowrap", gap: "8px" }}>
                ...
                <IconButton
                  color='error'
                  onClick={() => {
                    handleDelete("income", row.original.id);
                  }}
                >
                  <DeleteIcon />
                </IconButton>
